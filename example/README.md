...
- The repository README still mentions `example/input.yaml`, `example/rule_based.yaml`, `example/phase_field.yaml`, and `example/hoss.yaml`.
- Those files are not present in the current `example/` directory.
- The current example YAML filenames are:
  - `train_rule_based.yaml`
  - `train_phase_field.yaml`
  - `train_mixed.yaml`
  - `test_phase_field.yaml`
...
- `train_rule_based.yaml` and `train_phase_field.yaml` are the validation defaults used by `example/validate_config.py`.
- `test_phase_field.yaml` is the default base config for `example/run_all_phase_tests.py`.
- `train_mixed.yaml` introduces `sim_type: MIXED` and `sim_weights`.
- None of the present `example/*.yaml` files appear structurally invalid based on the loader’s supported sections and keys.
- The main documentation in `README.md` is out of date with respect to example file names.

## Recommended mapping from old README names to current files

- `example/rule_based.yaml` → `example/train_rule_based.yaml`
- `example/phase_field.yaml` → `example/train_phase_field.yaml`
- `example/input.yaml` → not present; possibly an older minimal starter config.
- `example/hoss.yaml` → not present in current folder; may be an old HOSS example.
